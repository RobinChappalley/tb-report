...
for i in {1..12}; do
    ...
    for (( run=1; run<=NUMBER_OF_RUNS; run++ )); do
        ...
        # shuffle_array imprime une valeur par ligne -> lire en tableau sans mapfile (bash 3.2)
        shuffled_solutions=()
        while IFS= read -r solution; do
            shuffled_solutions+=("$solution")
        done < <(shuffle_array "${solutions[@]}")
        for sol in "${shuffled_solutions[@]}"; do
            ...
            metrics=$(curl -s -H "${ACCEPT_HEADER}" -o /dev/null -w '%{http_code},%{content_type},%{size_download},%{time_starttransfer},%{time_total},%{cf-cache-status}' "${target_url}")
