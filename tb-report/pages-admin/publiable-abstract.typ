...
== Résultats
Le benchmark retient l'outil open-source ``` imgproxy```, placé derrière un serveur reverse-proxy Nginx configuré pour la mise en cache HTTP. Les images sont désormais générées uniquement à la demande de l'utilisateur final. L'architecture élimine la pré-génération d'images à l'upload, réduit l'espace disque consommé et garantit des coûts d'infrastructure prévisibles, indépendants du volume de médias transformés. En supprimant la configuration manuelle du redimensionnement sur chaque CMS, la solution améliore l'expérience développeur (DX) et réduit le nombre de tâches répétitives]